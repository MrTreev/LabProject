...
This HDMI transmittor IC simplifies the logic required to output a valid HDMI video signal. The IC, configurable via \IsqrC, supports vairous HDMI resolutions and colour formats. When correctly configured, ADV7513 receives a video signal from the FPGA, and outputs it through an HDMI connector, where it is interpreted by an HDMI monitor.
...
For this project, a video mode of $640 \times 480$ @ \SI{60}{Hz} is used, as it is simple and supported by most (if not all) monitors. This mode requires the video signal timing periods described in Table \ref{Video480Timing}. The frequency of the video clock can also be inferred from this information, as the total number of pixels for each frame is $800 \times 525 = 420000$, therefore $420000 \times 60 = 25200000$ pixels per second, translating to a \SI{25.2}{MHz} clock.
...
\textcite{ADVProgrammingGuide} describes various registers that can be set to achieve any desired configuration. However, due to its lengthiness and complexity, it was deemed easier to reference an example design provided with \textcite{DE10Manual}. Their HDMI\_TX example uses the same colour format as this project, and is tested to be functional. Table \ref{ADVRegisters} lists the registers the example code sets, along with their comments. These registers also includes configuration of sound for the ADV7513, which remained unused in this project, but the \IsqrC configuration remained nontheless.
...
The input FPGA\_CLK1\_50 is connected to the built-in \SI{50}{MHz} clock of the FPGA; BTN0 is connected to 1 of the 2 tactile buttons on the DE-10 Nano, it is debounced by external circuits on the board. The outputs named ADV\_\* all connect to the ADV7513 transmittor IC. Internal signals are denoted in lower case.
...
\begin{description}
	\item[i2c\_clk] The \IsqrC interface of ADV7513 does not function with a high frequency, therefore the clock is divided by 4096 into \SI{12.20703125}{kHz} before being used by the \IsqrC controller.
	\item[ADV\_CLK] As established in Section \ref{ADV7513}, the ADV7513 transmittor IC requires a \SI{25.2}{MHz} clock signal. This can be approximated by dividing FPGA\_CLK1\_50 by 2 to obtain a \SI{25}{MHz} clock.
	\item[pix\_clk] As shown in Figure \ref{VideoClockData}, ADV\_CLK must not be the same as the clock that updates D[23:0], instead, ADV\_CLK needs to be \ang{90} earliar in phase than this clock.
\end{description}
...
The phase shift between ADV\_CLK and pix\_clk are created by inverting the FPGA\_CLK1\_50 for the clock divider generating ADV\_CLK, but not inverting the other. The result is illustrated in Figure \ref{ClockDividerPhaseShift}.
...
The clock divider used to be able to divide clocks by an arbitrary factor, where the output is produced by comparing the counter content to half of the dividing factor. However, doing so appears to prompt difficult-to-track issues, thus the design is simplified to only divide by a power of 2, and renamed from ClockDivider to ClockDividerPow2.

ADV\_CLK and pix\_clk used to be more closely approximated to the ideal \SI{25.2}{MHz} frequency, derived with a PLL. This is also ditched as the PLL also prompts difficult-to-track issues within the generated clocks, and a simplier \SI{25}{MHz} clock appears to be functional.

## PixelCursor

2 counters are used to generated the video control ignals of DE, Vsync and Hsync, as specified in Figure \ref{VideoTimingGraph} and Table \ref{Video480Timing}. The counter hcount stores the horizontal coordinate of the pixel being drawn, increases on every rising edge of pix\_clk, and its value $c_h \in \mathbb{Z} \cap [0, 799]$. The counter vcount stores the vertical coordinate of the pixel being drawn, increases whenever hcount overflows from 799 to 0, and its value $c_v \in \mathbb{Z} \cap [0, 524]$. For simplicity, the $(0, 0)$ position is defined to be at the top-left corner of active frame data, after the horizontal and vertical front porches (Figure \ref{PixelCursorCoordinate}). This effectively moves the front porches to be positioned after the sync periods, from the perspective of the signal receiver, no difference is made, but from the perspective of within the FPGA design, this decision has the advantage of hcount and vcount directly corresponds to the coordinates of the actual pixel being displayed, when video is active ($c_h < 640$ and $c_v < 480$). Therefore, their values can be directly output as pix\_x and pix\_y, which are received by PixelAddr and PixelStream to decide what pixel colours to draw.
...
\textcite{Astley1987} is cropped and scaled to a resolution of $160 \times 120$, each frame of the video is extracted as 19200 raw pixel values, in 24-bit RGB format which can be directly output to the ADV7513. Initially, a raw binary file containing all frames in 10 frames per second was planned to be stored onto a Micro-SD card. The controller software would copy each frame of the video into RAM consecutively every \SI{0.1}{s}, where it is read by the FPGA and sent to the ADV7513. However, as the project scopes was reduced, video data needed to be stored on the FPGA itself. Since the memory available on the FPGA is severely limited, only 2 frames is fit into the module.
...
The downsides to this approach are severely increased compilation time and logical element usage. An attempt at mitigating these issues was made, by using a ROM IP block. An additional advantage to that approach is more available memory space, which can accomodate more than 5 frames. However, a ROM IP block requires 3 clock cycles for its output to respond to the input address, making it unfit for the current design (Figure \ref{ROMIPOutput}).

\begin{figure}
	\centering
	\begin{tikztimingtable}[timing/wscale=5]
		Clock & [L] 6{0.5H0.5L} \\
		Address & [X] D{0} D{1} D{2} D{3} D{4} D{5} \\
		Desired output & [X] D{mem[0]} D{mem[1]} D{mem[2]} D{mem[3]} D{mem[4]} D{mem[5]} \\
		Actual output & [X] U{} U{} D{mem[0]} D{mem[1]} D{mem[2]} D{mem[3]} \\
	\end{tikztimingtable}
	\caption{Desired and actual output of PixelStream if it used an ROM IP block}
	\label{ROMIPOutput}
\end{figure}
